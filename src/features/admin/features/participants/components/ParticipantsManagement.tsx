...
import { ParticipantMobileBulkBar, ParticipantMobileList } from "./mobile";
...
export function ParticipantsManagement() {
  ...
  const participantSortOptions = useMemo(
    () => [
      ...
      { key: "actions", label: "Review priority" },
    ],
    [],
  );
  ...
  function renderParticipantReviewButton(team: ParticipantRow) {
    const needsReview = registrationNeedsReview(team.status, team.tournamentStatus);
    return (
      <Button
        type="button"
        size="sm"
        variant="outline"
        className={cn(
          "min-h-11 min-w-[5.5rem] gap-1.5 font-tech text-[10px] uppercase tracking-wider-2 sm:min-h-9",
          needsReview
  ...
  return (
    ...
    >
      ...
      <div className="p-4 pt-4 sm:p-6">
        ...
        {selectedCount > 0 && (
          <div className="mb-4 hidden flex-wrap items-center justify-between gap-3 border border-border bg-muted/10 px-4 py-3 md:flex">
            ...
            <ul className="divide-y divide-white/8 md:hidden">
              {Array.from({ length: 5 }).map((_, i) => (
                <li key={i} className="flex items-start gap-3 px-4 py-4">
                  ...
                  <Skeleton className="h-11 w-16 shrink-0 rounded-none" />
                </li>
              ))}
            </ul>
          ) : (
            ...
            {filteredParticipants.length === 0 ? (
              ...
              <>
                <ParticipantMobileList
                  participants={pagination.paginatedItems}
                          ...
                          <TableRow key={team.id}>
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end">
                                {renderParticipantReviewButton(team)}
                              </div>
                            </TableCell>
                          </TableRow>
                        );
