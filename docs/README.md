...
| 3 | `features/admin/features/tournaments/` | Create/list events |
| 4 | `features/admin/features/participants/` | Approve/reject team sign-ups |
| 5 | `features/admin/features/tournament/` | Seed bracket, scores, publish |
...
| Column | Type | Notes |
|--------|------|--------|
| `id` | uuid | **PK** (match `auth.users.id` if using Supabase Auth) |
| `username` | text | unique |
| `discord_username` | text | unique |
| `discord_id` | text | nullable, unique |
| `role` | text | `User` \| `Tournament Admin` \| `Super Admin` |
...
|-----------|------|-------|
| `members/services/members.service.ts` | `profiles` | insert `profiles` |
...
4. Enable **RLS** — admin mutations for `Tournament Admin` and `Super Admin` on `profiles.role`.
5. Set env vars and swap mock services for `@supabase/supabase-js`:
